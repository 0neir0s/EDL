...
{
	TR0 = 0;
	time_meas = TH0*256+TL0+overflow_count*65536;
	if(time_meas>0 && time_meas<2300)
	{
		if(time_meas>1200)
		{
			RX_char[i_count] = '0';
...
void print_char()
{
	RX_char[9] = (char) (128*bit_array[7]+64*bit_array[6]+32*bit_array[5]+16*bit_array[4]+8*bit_array[3]+4*bit_array[2]+2*bit_array[1]+bit_array[0]);
}